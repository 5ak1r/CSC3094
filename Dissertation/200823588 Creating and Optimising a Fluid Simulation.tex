...
\begin{document}
    ...
    Game engines have the advantage of simplicity, leading to a faster development cycle. There are a lot of available pre-built tools, editors and assets that allow rapid prototyping and game development. For this dissertation, most of these tools are not as necessary; the physics engine for example would not be utilised in this project since all the physics will be manually written. They also abstract away from the low-level graphics rendering pipeline and handle it automatically.
